...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """YOUR CODE"""
        inActs.insert(0, 1)
        wSum = self.getWeightedSum(inActs)
        value = self.sigmoid(wSum)
        inActs.pop(0)
        return value
    ...
    def sigmoidDeriv(self, value):
        ...
        """
        """YOUR CODE"""
        answer = (1 - self.sigmoid(value)) * (self.sigmoid(value))
        return answer
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        """YOUR CODE"""
        answer = self.sigmoidDeriv(self.getWeightedSum([1] + inActs))
        return answer
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        let = 0
        while let < length:
            future_weight = self.weights[let] + (alpha * delta * inActs[let])
            future_weight_list.append(future_weight)
            totalModification = totalModification + future_weight_list[let] - self.weights[let]
            let = let + 1
        self.weights = future_weight_list
        if 0 > totalModification: 
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for lyr in self.layers:
            outcome.append(inputs)
            
            inputs = [prction.sigmoidActivation(inputs) for prction in lyr]
        outcome.append(inputs)
        return outcome
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    for n in range(len(nextLayer)):
                        delta = deltas[0][n] * nextLayer[n].weights[1 + neuronNum]
                        err = err + delta
                    delta = err * gPrime
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    """
    while weightChangeThreshold < weightMod and maxItr > iteration:
        iteration = iteration + 1
        weightMod = nnet.backPropLearning(examplesTrain, alpha)[1]
        if (iteration % 10) == 0:
           print('iteration '+str(iteration)+': trainerror '+str(trainError)+' weight change '+str(weightMod))
        else:
            print('\n')
    ...
    for let in examplesTest:
        same = 1
        output = nnet.feedForward(let[0])[-1]
        length = len(output)
        num = 0;
        while num < length:
            if round(output[num]) != let[1][num]:
                same = 0
            num = num + 1;
        if same:
